...
export class NotebookAPI {
  ...
  saveToNotebookMetadata(key: string, value: any) {
    this.panel.model.setMetadata(key, value);
  }
  ...
  get language(): string | undefined {
    return this.notebook?.model?.getMetadata('language_info')?.name;
  }
  ...
  public addCell(kind: 'code' | 'markdown', text: string, index?: number) {
    ...
    console.log('making cell');
      if (kind === 'code') {
        // this.notebook.contentFactory.createCodeCell({});
  ...
  private listenToCells() {
    ...
    this.notebook.model.metadataChanged.connect((_, args) => {
