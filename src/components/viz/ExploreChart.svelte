...
function splitOnNull(p) {
    return p[yAccessor] === undefined || p[yAccessor] === null || Number.isNaN(p[yAccessor]);
}
...
$: X = scaleTime().domain([$innerXMin, $innerXMax]).range([plotLeft, plotRight]);
$: Y = scaleLinear().domain([zeroBound ? 0 : yDomain[0], yDomain[1]]).range([plotBottom, plotTop]);
...
<svg {width} height={24}>
    {#each X.ticks(5) as xTick}
        ...
        <text x={X(xTick)} y={22}>{formattedTick === 'Jan 01' ? secondaryFmt(xTick) : formattedTick}</text>
...
>
    ...
    {#each X.ticks(5) as xTick}
        <line x1={X(xTick)} x2={X(xTick)} y1={0} y2={height} stroke=hsl(1,0%,80%) />
    {/each}
    {#each plotData as series}
        {@const datum = series[0]}
        {#if series.length > 5}
            <path clip-path="url(#explore-{cheapID})" d={lineGenerator(series)} fill=none stroke={color} />
            {#if zeroBound}
                <path clip-path="url(#explore-{cheapID})" d={areaGenerator(series)} fill=black opacity=.2 />
            {/if}
        {:else}
            <circle cx={X(datum[xAccessor])} cy={Y(datum[yAccessor])} r=1 fill={color} />
        {/if}
    {/each}
    ...
    {#if hoveredDate}
        ...
        {/if}
        <line x1={X(hoveredDate)} x2={X(hoveredDate)} y1={0} y2={height} stroke=black /> 
        {#if hoveredPoint}
            <circle cx={X(hoveredPoint[xAccessor])} cy={Y(hoveredPoint[yAccessor])} r=3 fill=black />
        {/if}
    {/if}
