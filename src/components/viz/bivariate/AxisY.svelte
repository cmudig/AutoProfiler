 ...
 <script>
  ...
  <g class='axis y-axis' transform='translate({-$padding.left}, 0)'>
    {#each tickVals as tick (tick)}
      <g class='tick tick-{tick}' transform='translate({$xRange[0] + $padding.left - 5}, {$yScale(tick)})'>
        {#if gridlines !== false}
          <line
            class="gridline"
            x2='100%'
    ...
    {/each}
    <g class="tick" transform="translate(10,{$yScale(tickVals[Math.floor(tickVals.length/2)])})">
